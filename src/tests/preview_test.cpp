...
void saveImage(const std::string& fn, qreal pos, std::string target) {
    Previewer previewer(fn, nullptr);
    auto image = previewer.previewRequest(pos);
    image.save((std::string("D:/test_video/") + target).c_str());
}

TEST(preview_test, saveImage) {
    std::string fn("D:/test_video/video.mp4");
    // 预览中间
    saveImage(fn, 0.0, "left.jpg");
    saveImage(fn, 5.0, "middle.jpg");
    saveImage(fn, 12.0, "right.jpg");
    // 预览越界
    saveImage(fn, -1.0, "beyond_left.jpg");
    saveImage(fn, 20.0, "beyond_right.jpg");
}